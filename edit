...
<body>
  <header>
    ...
    <h1>Редактор игры «Парящий квиз» — КГБ</h1>
  </header>
...
<select id="font">
  ...
  <option>Noto Sans</option>
      ...
      <div class="row">
        ...
        <div>
          <label>Кастомный курсор (URL, опц.)</label>
      ...
      <div class="card" style="margin-top:16px">
        ...
        <div class="preview-controls">
          <input type="text" id="previewValue" placeholder="Текст предпросмотра или URL картинки..." value="Preview">
          <div>
            <button type="button" class="btn-ghost" id="btnText">Текст</button>
            <button type="button" class="btn-ghost" id="btnImage">Картинка</button>
          </div>
        </div>
        ...
        <div class="hint">Если указан URL — показывается картинка/гифка. Шрифт/цвет/размер применяются к тексту предпросмотра.</div>
      </div>
      ...
      <div class="card" style="margin-top:16px">
        ...
        <div class="row">
          <div>
            <label>Вариантов на вопрос (2–10)</label>
            <input type="number" id="answersPerQuestion" value="4" min="2" max="10">
          </div>
          <div>
            <label>Интервал спавна (мс)</label>
    ...
    <section class="card">
      ...
      <div class="toolbar">
        <button id="addQ">➕ Добавить вопрос</button>
        <span class="pill">Неограниченно вопросов</span>
      </div>
      ...
      <div class="result" style="margin-top:10px; display:grid; grid-template-columns:1fr auto; gap:12px; align-items:start">
        <textarea id="iframeOut" readonly placeholder="Здесь появится iframe для вставки" style="height:120px; font-family:ui-monospace, SFMono-Regular, Menlo, Consolas, monospace"></textarea>
        <button id="copyBtn" class="btn-ghost">📋 Копировать</button>
      </div>
      <div class="hint">Загрузи сохранённый JSON в папку <code>/data</code> своего репозитория → Commit.</div>
    </section>
  </main>

  <footer>© 2025 <a href="https://kgbgamecorp.ru" target="_blank" rel="noopener">Корпорация Геймификации Будущего</a></footer>
...
function applyFontToPreview(){
  ...
  text.style.color      = $('#textColor').value;
    ...
    ['font','fontSize','textColor','previewValue'].forEach(id=>{
    ...
    function qTemplate(idx){
      ...
      wrap.innerHTML=`
        <h3>Вопрос <span class="num">${idx+1}</span>
          <span class="right">
        ...
        <label>Текст вопроса (вверху)</label>
        <input type="text" class="question" placeholder="Например: Столица Франции?">
        <div class="row">
          <div>
            <label>Правильный ответ (текст или URL)</label>
            <input type="text" class="correct" placeholder="Париж или https://.../cat.gif">
          </div>
          <div>
            <label>Неправильные (по одному в строке)</label>
            <textarea class="wrong" placeholder="Марсель\nЛион\nНицца"></textarea>
          </div>
        </div>`;
    ...
    function collect(){
      ...
      return {
        ...
        speed: Math.max(1, Math.min(10, parseInt($('#speed').value||'5',10))), // 1..10
    ...
    function validate(cfg){
      if(!cfg.questions.length) return 'Добавьте хотя бы один вопрос.';
      for(const [i,q] of cfg.questions.entries()){
        if(!q.question) return `Вопрос ${i+1}: заполните текст вопроса.`;
        if(!q.correct)  return `Вопрос ${i+1}: укажите правильный ответ.`;
        if((q.wrong?.length||0) + 1 < 2) return `Вопрос ${i+1}: нужен минимум один неправильный.`;
      }
      return null;
    }
    ...
    async function publish(){
      ...
      alert('JSON сохранён. Загрузите файл в /data вашего репозитория.');
    }

    function quickTest(){
      const cfg=collect(); const err=validate(cfg); if(err){ alert(err); return; }
      alert('JSON валиден. Вопросов: '+cfg.questions.length);
    }
    function copyIframe(){
      const v=$('#iframeOut').value.trim(); if(!v){ alert('Сначала опубликуйте, чтобы получить iframe.'); return; }
